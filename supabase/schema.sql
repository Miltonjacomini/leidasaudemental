...
-- Insert seed data (the original hardcoded posts)
INSERT INTO public.blog_posts (title, excerpt, date, read_time, tags, gradient) VALUES
(
  'Building Scalable React Applications with TypeScript',
  'Learn how to structure large React applications using TypeScript, exploring advanced patterns, state management, and best practices for maintainable code.',
  'Dec 15, 2024',
  '8 min read',
  ARRAY['React', 'TypeScript', 'Architecture'],
  'bg-gradient-to-br from-blue-500 to-purple-600'
);

-- Create enum for blog content status
CREATE TYPE blog_content_status AS ENUM ('draft', 'ready', 'archived');

-- Create the blog_content table
CREATE TABLE IF NOT EXISTS public.blog_content (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  id_blog_content UUID NOT NULL REFERENCES public.blog_posts(id) ON DELETE CASCADE,
  content TEXT NOT NULL,
  status blog_content_status NOT NULL DEFAULT 'draft',
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Create policy to allow public read access for ready content
CREATE POLICY "Allow public read access for ready content" ON public.blog_content
  FOR SELECT
  USING (status = 'ready');
...
-- Create index for faster lookups by blog post id
CREATE INDEX idx_blog_content_blog_post_id ON public.blog_content(id_blog_content);

-- Create index for status filtering
CREATE INDEX idx_blog_content_status ON public.blog_content(status);

-- Insert blog post about incident management and SRE
WITH new_post AS (
  INSERT INTO public.blog_posts (title, excerpt, date, read_time, tags, gradient) VALUES
  (
    'Tolerância a falhas em Big Tech: Como funciona o processo de incidentes no Mercado Livre',
    'Descubra como funciona o processo estruturado de SRE e gestão de incidentes em uma Big Tech, desde o acionamento da Guardia até a criação de postmortens detalhados.',
    'Jan 15, 2025',
    '10 min read',
    ARRAY['SRE', 'Incident Management', 'Big Tech', 'Postmortem', 'Microservices', 'DevOps'],
    'bg-gradient-to-br from-red-500 via-orange-500 to-yellow-600'
  )
  RETURNING id
)
INSERT INTO public.blog_content (id_blog_content, content, status)
SELECT 
  new_post.id,
